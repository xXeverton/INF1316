...
// Protótipos das funções principais que são usadas em outros arquivos além delas mesmas
void run_controller(int write_fd);      // gera interrupções
void run_kernel(int read_fd, int write_fd);  // gerencia os processos
...
typedef struct {
    int id_processo; // Qual processo é dono deste quadro (0 a 4) A1 a A5.
    int pagina_logica; // identificador da página em caso de swapping para alterar o modifyBit.
} QuadroRAM;
