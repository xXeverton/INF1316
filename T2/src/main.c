...
/*
 * SIMULADOR DE MICRO-KERNEL CLIENTE UDP (INF1316 - T2)
 * 
 ...
 * 2. KernelSim - Gerencia 5 aplicações e atua como cliente UDP de um SFSS.
 * 
 * Alterações de contexto ocorrem quando:
 * - IRQ0 (TimeSlice a cada 500ms): o Kernel escolhe outro processo para executar
 * - IRQ1/IRQ2: respostas de Arquivos e Diretórios vindas da rede são entregues
 */
...
int main()
{
    int fd[2]; // Pipe sempre precisa de tamanho 2
    ...
    printf("Iniciando o Simulador de Micro-Kernel (T2)...\n");
    ...
    if (pid_controller == 0)
    {
        ...
        close(fd[0]);
        run_controller(fd[1]); // Passa o fd de escrita
        exit(0);
    }
    else 
    {
        // CÓDIGO DO PAI: KernelSim
        // O pai gerencia a execução e lê as interrupções do filho
        run_kernel(fd[0], fd[1]); // Passa leitura e escrita (para os apps)
